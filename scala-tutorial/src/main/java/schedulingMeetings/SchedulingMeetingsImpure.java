package schedulingmeetingsimpure;
import java.util.*;
...
public class SchedulingMeetingsImpure {
	 ...
	 * <p>
	 * We use MeetingTime defined here in both Java and Scala versions. Note this is
	 * written in Java, but in the spirit of immutability.
	 * <p>
	 * In Scala, this looks like: case class MeetingTime(startHour: Int, endHour:
	 * Int)
	 */
	public static class MeetingTime { // or: record MeetingTime(int startHour, int endHour) {};
	...
	/*
	 * PREREQUISITE 2: unsafe functions that we'll use to simulate external API
	 * calls
	 *
	...
	/**
	 * Less evil version of calendarEntriesApiCall used to show the business logic
	 * of an imperatively written schedule function.
	 */
	 ...
	 *
	 * For demonstration purposes we are showing a slightly different version than
	 * in the book. It's a less evil version of the proper imperative schedule
	 * function (below) used to show happy path of the business logic. API calls
	 * don't throw any errors, but they still may return different results for the
	 * same parameters (randomly).
	 */
	static MeetingTime scheduleNoFailures(String person1, String person2, int lengthHours) {
		List<MeetingTime> person1Entries = calendarEntriesApiCallNoFailures(person1); // this is different than in the
																						// book
	...
	/**
	 * Proper version of imperatively written schedule function with one-retry
	 * recovery strategy. Uses calendarEntriesApiCall that may fail.
	 */
